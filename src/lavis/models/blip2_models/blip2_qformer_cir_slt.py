...
@registry.register_model("blip2_cir_slt")
class Blip2QformerCirSlt(Blip2Base):
    """
    ...
    def __init__(
        ...
        self.new_head = nn.Linear(128,128)
    ...
    def forward(self, samples):
        ...
        # fusion output selection
        _, indeces = torch.topk(fusion_output.attentions[-1][:,:,32, :32].mean(1), num_prompt)
        selected_output = torch.gather(fusion_output.last_hidden_state[:, : query_tokens.size(1), :], 1, indeces.unsqueeze(-1).repeat(1, 1, 768))
    
        slt_atts = torch.ones(selected_output.size()[:-1], dtype=torch.long).to(
            self.device
        )
        attention_mask = torch.cat([slt_atts, text_tokens.attention_mask], dim=1)
        text_output = self.Qformer.bert(
            text_tokens.input_ids,
            query_embeds=selected_output,
            attention_mask=attention_mask,
            return_dict=True,
        )

        fusion_feats = F.normalize(
            self.text_proj(text_output.last_hidden_state[:, num_prompt, :]), dim=-1
        )
    ...
    def inference(self, reference_embeds, target_feats, text):
        ...
        _, indeces = torch.topk(fusion_output.attentions[-1][:,:,32, :32].mean(1), 16)
        selected_output = torch.gather(fusion_output.last_hidden_state[:, : query_tokens.size(1), :], 1, indeces.unsqueeze(-1).repeat(1, 1, 768))
        ...
        fusion_feats = F.normalize(
            self.text_proj(text_output.last_hidden_state[:, 16, :]), dim=-1
        )
