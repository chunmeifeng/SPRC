...
@registry.register_model("blip2_cir_neg_rel")
class Blip2QformerCirNegRel(Blip2Base):
    """
    ...
    def forward(self, samples):
        ...
        #     image.device
        # )
        rel_tokens = self.query_tokens.expand(image_embeds.shape[0], -1, -1)
        img_output = self.Qformer.bert(
            query_embeds=rel_tokens,
            encoder_hidden_states=image_embeds,
            encoder_attention_mask=target_atts,
            use_cache=True,
            return_dict=True,
        )
        rel_output = self.Qformer.bert(
            query_embeds=img_output.last_hidden_state,
            encoder_hidden_states=taregt_embeds,
            encoder_attention_mask=target_atts,
            use_cache=True,
            return_dict=True,
        ...
        rel_feats = F.normalize(
            self.vision_proj(rel_output.last_hidden_state), dim=-1
        )
        sim_r2t = torch.matmul(
            text_only_feat.unsqueeze(1).unsqueeze(1), rel_feats.permute(0, 2, 1)
        ).squeeze()
