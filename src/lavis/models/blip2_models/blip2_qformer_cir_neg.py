...
@registry.register_model("blip2_cir_neg")
class Blip2QformerCirNeg(Blip2Base):
    """
    ...
    def forward(self, samples):
        ...
        fusion_embeds = fusion_output.last_hidden_state[:, : query_tokens.size(1), :]
        text_output = self.Qformer.bert(
            text_tokens.input_ids,
            query_embeds=fusion_embeds,
            attention_mask=attention_mask,
            return_dict=True,
        )
        ...
        ###============== Fusion-target Matching ===================###
        target_d = taregt_embeds.detach()
        fusion_d = fusion_embeds.detach()
        ...
        for b in range(bs):
            ...
            target_embeds_neg.append(target_d[neg_idx])
        ...
        for b in range(bs):
            ...
            reference_neg.append(fusion_d[neg_idx])
        ...
        query_fusion = torch.cat(
            [fusion_embeds, fusion_embeds, reference_neg], dim=0
        )  # pos, pos, neg

        query_tokens_itm = self.query_tokens.expand(text_ids_all.shape[0], -1, -1)

        query_tokens_itm = torch.cat([query_fusion, query_tokens_itm], dim=1)
        query_atts_itm = torch.ones(query_tokens_itm.size()[:-1], dtype=torch.long).to(
            image.device
        )
        # query_atts_itm = torch.ones(query_fusion.size()[:-1], dtype=torch.long).to(
        #     image.device
        # )
    ...
    def grad(self, samples):
        ...
        query_fusion = fusion_embeds
        query_tokens_itm = self.query_tokens.expand(text_tokens.input_ids.shape[0], -1, -1)
    ...
    def inference_rerank(self, refereence_embeds, target_embeds, text):
        ...
        # get (ref + caption) fusion sentence prompts
        # query tokens
        image_atts = torch.ones(refereence_embeds.size()[:-1], dtype=torch.long).to(
            refereence_embeds.device
        )
        query_tokens = self.query_tokens.expand(refereence_embeds.shape[0], -1, -1)
        query_atts = torch.ones(query_tokens.size()[:-1], dtype=torch.long).to(
            self.device
        )
        attention_mask = torch.cat([query_atts, text_atts], dim=1)
        fusion_output = self.Qformer.bert(
            ...
            query_embeds=query_tokens,
            attention_mask=attention_mask,
            encoder_hidden_states=refereence_embeds,
            encoder_attention_mask=image_atts,
            return_dict=True,
        )
        fusion_embeds = fusion_output.last_hidden_state[:, : query_tokens.size(1), :] 
        # got sentence prompts 

        # ITM
        query_tokens_itm = torch.cat([fusion_embeds, query_tokens], dim=1)
        query_atts_itm = torch.ones(query_tokens_itm.size()[:-1], dtype=torch.long).to(
            refereence_embeds.device
        )  
