...
@registry.register_model("blip2_cir_cat")
class Blip2QformerCirCat(Blip2Base):
    """
    ...
    def inference_rerank(self, refereence_embeds, target_embeds, text):
        ...
        """
        refereence_embeds = refereence_embeds.cuda()
        ...
        target_atts_all = torch.ones(refereence_embeds.size()[:-1], dtype=torch.long).to(
        ...
        text_output = self.Qformer.bert(
            text_inputs,
            query_embeds=fusion_output.last_hidden_state[:, : 32, :],
            attention_mask=attention_mask_all,
            return_dict=True,
        )
        ...
        sim_t2q = torch.matmul(target_embeds, fusion_feats.unsqueeze(-1)).squeeze()   
    ...
    def inference_gis(self, image, target, text):
        # image.shape = bs * 3 * 224 * 224
        # target.shape = bs * k * 3 * 224 * 224
        ...
        # fusion_feats.shape = bs * 768
        # target_feats = (bs * n) * 32  * 768
