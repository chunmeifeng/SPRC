...
class Blip2QformerCirRerank(Blip2Base):
    ...
    def __init__(
        ...
        # ------ frozen q former
        self.Fformer, _ = self.init_Qformer(
            num_query_token, self.visual_encoder.num_features, cross_attention_freq
        )
        ...
        for name, param in self.Fformer.named_parameters():
            if "_query" in name:
                key_orig = name.replace("_query", "")
                param.data.copy_(state_dict[key_orig])
        for param_q, param_k in zip(self.Qformer.parameters(), self.Fformer.parameters()):
        ...
        self.query_tokens_f = nn.Parameter(
            torch.zeros(1, num_query_token, self.Qformer.config.hidden_size)
        ) 
        self.query_tokens_f.data.copy_(self.query_tokens.data) 
        self.query_tokens_f.requires_grad = False

        self.vision_proj_f = nn.Linear(self.Qformer.config.hidden_size, embed_dim)
        self.text_proj_f = nn.Linear(self.Qformer.config.hidden_size, embed_dim)
        for param_q, param_k in zip(self.vision_proj.parameters(), self.vision_proj_f.parameters()):
            param_k.data.copy_(param_q.data)  # initialize
            param_k.requires_grad = False  #
        for param_q, param_k in zip(self.text_proj.parameters(), self.text_proj_f.parameters()):
            param_k.data.copy_(param_q.data)  # initialize
            param_k.requires_grad = False  #
        # ------ frozen q former
    ...
    def forward(self, samples):
        ...
        # image features
        query_tokens_f = self.query_tokens_f.expand(image_embeds.shape[0], -1, -1)
        target_output = self.Fformer.bert(
            query_embeds=query_tokens_f,
        ...
        target_feats = F.normalize(
            self.vision_proj_f(target_output.last_hidden_state), dim=-1
        )
        # text features
        ...
        text_feat = F.normalize(
            self.text_proj_f(text_output.last_hidden_state[:, 0, :]), dim=-1
        )
        # get score from relative_caption and target image
        sim_t2q = torch.matmul(
            text_feat.unsqueeze(1).unsqueeze(1), target_feats.permute(0, 2, 1)
        ).squeeze()
