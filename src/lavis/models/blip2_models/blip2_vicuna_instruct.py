...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def __init__(
        ...
        self.l2q_proj = nn.Linear(
            self.llm_model.config.hidden_size, self.Qformer.config.hidden_size
        )
    ...
    def forward(self, samples):
        ...
        ref_feats = F.normalize(
            self.l2q_proj(hidden_states).mean(1)
        )
        # 
        ...
        target_feats = F.normalize(
            target_output.last_hidden_state.mean(1), dim=-1
        )
        ...
        sim_i2t = ref_feats @ target_feats.t()
    ...
    def inference(self, reference_embeds, target_feats, text):
        ...
        inputs_llm = self.llm_proj(query_output.last_hidden_state[:,:query_tokens.size(1),:])
        atts_llm = torch.ones(inputs_llm.size()[:-1], dtype=torch.long).to(reference_embeds.device)
    ...
    def extract_target_features(self, image, mode='mean'):
        ...
        # return image_embeds
        image_features = F.normalize(image_embeds.mean(1), dim=-1)
